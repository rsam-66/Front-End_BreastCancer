...
// Import Icons
import PatientIcon from "@/assets/admin/patient.png"; // Fallback or use specific
...
import ImageStatsIcon from "@/assets/admin/image.png"; // Assuming exists or fallback
...
const isAttentionCase = (p) => {
  const isPending = p.review === "PENDING" || p.review === "-" || p.review === "Not Yet";
  ...
  if (p.medical_records && p.medical_records.length > 0) {
    ...
    if (latest && latest.ai_diagnosis) {
      ...
      return diag.includes("malignant"); // Simplistic check
    }
  }
  return false;
};
...
const stats = computed(() => {
  ...
  // Total Images (Assuming 1 record = 1 image for simplicity or count records)
  // Logic: Count patients with images
  const totalImages = patients.value.filter(p => p.image).length; // Or use strict record count
  ...
  return [
    ...
    {
      label: "Total Images",
      value: totalImages, // Or use total uploaded images
      icon: ImageStatsIcon,
      isBlue: true // Marker to apply blue style
    },
    {
...
const getAiAnalysisText = (p) => {
  // Return: Normal, Benign, Malignant, or Warning Icon
  if (p.medical_records && p.medical_records.length > 0) {
    const latest = p.medical_records[p.medical_records.length - 1];
    if (latest && latest.ai_diagnosis) {
      let d = "";
      try {
        const parsed = JSON.parse(latest.ai_diagnosis);
        d = parsed.class || latest.ai_diagnosis;
      } catch (e) { d = String(latest.ai_diagnosis); }

      d = d.toLowerCase();
      if (d.includes("normal")) return { text: "Normal", class: "text-green-500 font-bold" };
      if (d.includes("benign")) return { text: "Benign", class: "text-[#0099ff] font-bold" };
      if (d.includes("malignant")) return { text: "Malignant", class: "text-red-500 font-bold" };
    }
  }
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
      <div v-for="stat in stats" :key="stat.label"
        ...
        <div class="flex items-center gap-4 z-10">
          <!-- Icon Circle? No, design shows just icon -->
        ...
        <div v-for="patient in filteredPatients" :key="patient.id"
          ...
          <div class="col-span-3 w-full flex justify-center">
            <span :class="getAiAnalysisText(patient).class">
              ...
              <span v-if="getAiAnalysisText(patient).text === 'Attention'" class="inline-block ml-1">⚠️</span>
            </span>
          </div>
