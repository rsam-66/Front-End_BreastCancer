...
// Import dataService (adjust path if needed, using alias @/ is safest)
...
const props = defineProps(['id']); // Matches route param ':id'

// Alias for easier usage if code uses patientId
const patientId = props.id;

const isLoading = ref(true);
const viewMode = ref('raw'); // 'raw' | 'normalized'
...
// Images
const currentImageSrc = ref(defaultCystImage); // Editable Base
const aiResultImageSrc = ref(defaultCystImage); // AI Reference
const patientData = ref({});
const aiPrediction = ref("Malignant"); // Default, will fetch
...
onMounted(async () => {
  if (props.id) {
    try {
      ...
      if (data.aiGradCamImage) aiResultImageSrc.value = data.aiGradCamImage;
      else aiResultImageSrc.value = data.image; // Fallback to raw if no GradCam
...
const handleSave = async () => {
  ...
  const heatmapDataURL = medicalCanvasRef.value ? medicalCanvasRef.value.getStageDataURL() : null;
...
const handleImageUpdate = (newSrc) => {
  ...
  // If user manually changes image, maybe update AI result too or keep it? 
  // For now let's keep AI result as reference to original.
  doctorDrawings.value = [];
};

// HELPERS
const setBrushType = (type) => { brushType.value = type; };
const getBrushButtonClass = (type) => {
  ...
  if (!isActive) return base + " bg-white border-slate-200 text-slate-500 hover:bg-slate-50";
  switch (type) {
    ...
    case 'erase': return base + " bg-gray-100 border-gray-500 text-gray-700 shadow-sm";
  }
};
</script>
  ...
  <div class="h-full w-full bg-slate-50 flex flex-col overflow-hidden relative">
    ...
    <div id="workspace-container" class="flex-1 overflow-y-auto p-8 pb-32">
      ...
      <div class="text-center mb-8">
        <h1 class="text-2xl font-medium text-slate-600">Reviewing Case #{{ patientId }}</h1>
      </div>

      <div v-if="isLoading" class="flex justify-center p-12">
        <p class="text-slate-400 font-bold animate-pulse">Loading Patient Data...</p>
      </div>
      ...
      <div v-else class="flex flex-col lg:flex-row gap-8 max-w-7xl mx-auto">
          ...
          <div class="bg-gray-100 rounded-3xl p-8 mb-6 shadow-sm border border-slate-200 relative">
            ...
            <div class="flex flex-col xl:flex-row gap-8 justify-center items-start">
              ...
              <div class="flex flex-col items-center">
                ...
                <p class="mt-3 font-bold text-slate-600 uppercase tracking-widest text-xs">AI Result</p>
              </div>

              <!-- 2. DOCTOR ANNOTATION (Editable) -->
              <div class="flex flex-col items-center">
                <MedicalCanvas ref="medicalCanvasRef" :baseImageSrc="currentImageSrc" :brushType="brushType"
                  :brushSize="brushSize" :viewMode="viewMode" @update:drawings="(data) => doctorDrawings = data" />
                <!-- Note: View Mode Toggle was here, moved to Control Panel below -->
              </div>
              ...
              <div class="flex items-center gap-4 border-b border-slate-200 pb-6">
                ...
                <div class="flex bg-white rounded-lg p-1 border border-slate-200">
                  <button @click="viewMode = 'raw'" class="px-4 py-1.5 rounded-md text-sm font-bold transition-all"
                    :class="viewMode === 'raw' ? 'bg-[#0099ff] text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'">
                    Raw Pixels
                  </button>
                  <button @click="viewMode = 'normalized'"
                    class="px-4 py-1.5 rounded-md text-sm font-bold transition-all"
                    :class="viewMode === 'normalized' ? 'bg-[#0099ff] text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'">
                    Normalized
                  </button>
                </div>
                <p class="text-xs text-slate-400 ml-2">
                  *{{ viewMode === 'raw' ? 'Edit raw output blocks' : 'View smoothed heatmap' }}
                </p>
              </div>
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                  <button @click="setBrushType('normal')" :class="getBrushButtonClass('normal')">
                    <div class="w-3 h-3 bg-green-500 rounded-sm"></div> Low
                  </button>
                  <button @click="setBrushType('benign')" :class="getBrushButtonClass('benign')">
                    <div class="w-3 h-3 bg-yellow-400 rounded-sm"></div> Medium
                  </button>
                  <button @click="setBrushType('malignant')" :class="getBrushButtonClass('malignant')">
                    <div class="w-3 h-3 bg-red-600 rounded-sm"></div> High
                  </button>
            ...
            <div class="flex-1">
              ...
              <div class="flex items-center gap-4 text-sm font-bold text-slate-500">
                ...
                <label class="flex items-center gap-2 cursor-pointer select-none"><input type="radio" value="disagree"
                    v-model="doctorAgreement" class="accent-red-500 w-5 h-5"><span
                    :class="doctorAgreement === 'disagree' ? 'text-red-600' : ''">Disagree</span></label>
              </div>
            </div>
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="font-bold"
                    :class="savedDiagnosis.agreement === 'agree' ? 'text-green-500' : 'text-red-500'">{{
                      savedDiagnosis.agreement === 'agree' ? 'Agree' : 'Disagree' }}</span>
                </div>
                <div>
                  <span class="font-bold text-slate-600">Note:</span>
                  <p class="text-slate-500 text-sm mt-1">{{ savedDiagnosis.note || "No notes." }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <!-- RIGHT COLUMN: Patient Info & Reference -->
