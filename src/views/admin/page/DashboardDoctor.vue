...
const handleAddDoctor = (newDoctor) => {
    const id = doctorList.value.length + 1
    doctorList.value.push({ id, ...newDoctor })
...
const handleEditDoctor = (updatedDoctor) => {
    ...
    if (index !== -1) {
        doctorList.value[index] = { ...selectedDoctor.value, ...updatedDoctor }
    }
...
const handleDeleteDoctor = () => {
    doctorList.value = doctorList.value.filter(d => d.id !== selectedDoctor.value.id)
...
<template>
    <div class="w-full">
        ...
        <div v-if="filteredDoctorList.length > 0" class="flex flex-col gap-4">
            <div v-for="doctor in filteredDoctorList" :key="doctor.id"
                ...
                <div class="flex-1 w-full md:w-auto flex justify-between md:justify-center items-center mb-2 md:mb-0">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10  rounded-full flex items-center justify-center text-white">
                            <img :src="DoctorIcon" alt="Total Doctor" class="w-8 h-8 object-contain" />
                        </div>
                        <span class="text-gray-700 font-medium whitespace-nowrap">D{{ String(doctor.id).padStart(3, '0')
                            }}</span>
                    </div>
                </div>
                ...
                <div class="flex-1 w-full md:w-auto flex justify-between md:justify-center items-center mb-2 md:mb-0">
                    ...
                    <span class="text-gray-700 text-sm md:text-base">{{ doctor.email }}</span>
                </div>
