...
const handleAddPatient = (newPatient) => {
    const id = patientList.value.length + 1
    patientList.value.push({ id, ...newPatient })
    isAddModalOpen.value = false
}

const handleEditPatient = (updatedPatient) => {
    const index = patientList.value.findIndex(p => p.id === selectedPatient.value.id)
    if (index !== -1) {
        patientList.value[index] = { ...selectedPatient.value, ...updatedPatient }
    }
...
const handleUploadImage = (imageUrl) => {
    const index = patientList.value.findIndex(p => p.id === selectedPatient.value.id)
    if (index !== -1) {
        patientList.value[index].image = imageUrl
        // If uploading for the first time or updating, set review to 'Not Yet' if it was '-'
        if (patientList.value[index].review === '-') {
            patientList.value[index].review = 'Not Yet'
        }
    }
...
const handleDeletePatient = () => {
    patientList.value = patientList.value.filter(p => p.id !== selectedPatient.value.id)
...
<template>
    <div class="w-full">
        ...
        <div class="text-center mb-8">
            <h2 class="text-gray-500 font-medium text-lg">Leading-edge technology for better diagnosis</h2>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            ...
            <div
                ...
                <div class="flex items-start justify-between">
                    <div class="bg-blue-50 p-2.5 rounded-xl min-w-[48px] h-[48px] flex items-center justify-center">
                        <img :src="PatientIcon" alt="Total Patient" class="w-6 h-6 object-contain" />
                    </div>
                    <span class="text-gray-500 text-sm font-medium pt-1 text-right">Total Patient</span>
                </div>
                <div class="flex justify-end items-end">
                    <span class="text-4xl text-gray-800 font-normal leading-none mb-1">{{ stats[0]?.value || 0 }}</span>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-start justify-between">
                    <div class="bg-[#eef5fa] p-2.5 rounded-xl min-w-[48px] h-[48px] flex items-center justify-center">
                        <img :src="DoctorIcon" alt="Total Doctor" class="w-6 h-6 object-contain" />
                    </div>
                    <span class="text-gray-500 text-sm font-medium pt-1 text-right">Total Doctor</span>
                </div>
                <div class="flex justify-end items-end">
                    <span class="text-4xl text-gray-800 font-normal leading-none mb-1">{{ stats[1]?.value || 0 }}</span>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-start justify-between">
                    <div class="bg-green-50 p-2.5 rounded-xl min-w-[48px] h-[48px] flex items-center justify-center">
                        <img :src="ImageIcon" alt="Image Uploaded" class="w-6 h-6 object-contain" />
                    </div>
                    <span class="text-gray-500 text-sm font-medium pt-1 text-right">Image Uploaded</span>
                </div>
                <div class="flex justify-end items-end">
                    <span class="text-4xl text-gray-800 font-normal leading-none mb-1">{{ stats[2]?.value || 0 }}</span>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-start justify-between">
                    <div class="bg-red-50 p-2.5 rounded-xl min-w-[48px] h-[48px] flex items-center justify-center">
                        <img :src="WaitingIcon" alt="Waiting For Review" class="w-6 h-6 object-contain" />
                    </div>
                    <span class="text-gray-500 text-sm font-medium pt-1 text-right">Waiting For Review</span>
                </div>
                <div class="flex justify-end items-end">
                    <span class="text-4xl text-gray-800 font-normal leading-none mb-1">{{ stats[3]?.value || 0 }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="w-full flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            ...
            <div class="flex items-center gap-4 w-full md:w-auto">
                ...
                <div class="relative w-full md:w-80">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 absolute right-3 top-2.5 text-gray-500"
        ...
        <div v-if="filteredPatientList.length > 0" class="flex flex-col gap-4">
            <div v-for="patient in filteredPatientList" :key="patient.id"
                ...
                <!-- ID -->
                <div class="flex-1 w-full md:w-auto flex justify-between md:justify-center items-center mb-2 md:mb-0">
                    <span class="md:hidden font-bold">ID:</span>
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-lg flex items-center justify-center">
                            <!-- Bed icon for patient ID -->
                            <img :src="PatientIcon" alt="Patient ID" class="w-8 h-8 object-contain" />
                        </div>
                        <span class="text-gray-700 font-medium whitespace-nowrap">P{{ String(patient.id).padStart(3,
                            '0') }}</span>
                    </div>
                </div>
                ...
                <div class="flex-1 w-full md:w-auto flex justify-between md:justify-center items-center mb-2 md:mb-0">
                    ...
                    <span v-if="patient.review === 'Done'" class="text-green-500 font-bold">Done</span>
                    <span v-else-if="patient.review === 'Not Yet'" class="text-[#006699] font-bold">Not Yet</span>
                ...
                <div class="flex-1 w-full md:w-auto flex justify-between md:justify-center items-center mb-2 md:mb-0">
                    ...
                    <span v-if="patient.image" class="text-green-500 font-bold">Yes</span>
                    <span v-else class="text-red-500 font-bold">No</span>
                </div>

                <!-- Actions -->
                <div class="flex-1 w-full md:w-auto flex justify-center items-center gap-4 mt-2 md:mt-0">
                    <!-- Edit Button -->
                    <button @click="openEditModal(patient)"
                        ...
                        <img :src="EditIcon" alt="Edit" class="w-10 h-10 object-contain" />
                    </button>
                    <!-- Delete Button -->
                    <button @click="openDeleteModal(patient)"
                        class="w-8 h-8 flex items-center justify-center rounded hover:opacity-80 transition-opacity">
                        <img :src="DeleteIcon" alt="Delete" class="w-10 h-10 object-contain" />
                    </button>
                </div>
            </div>
        </div>
        ...
        <!-- Empty State -->
        <div v-else class="w-full flex flex-col items-center justify-center py-12 text-gray-500">
