...
const handleLogin = () => {
  ...
  if (email.value === 'admin@humic.com' && password.value === 'admin123') {
    ...
    emit('close')
    router.push('/admin')
  } else if (email.value === 'doctor@humic.com' && password.value === 'doctor123') {
    localStorage.setItem('userRole', 'doctor')
    emit('close')
    router.push('/doctor')
  } else {
    alert('Invalid credentials! Try admin@humic.com / admin123')
  }
}
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center px-4">
    ...
    <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md relative z-10 animate-fade-in-up">
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        ...
        <button type="submit"
          class="w-full bg-sky-500 hover:bg-sky-600 text-white font-bold py-3 rounded-full transition-colors cursor-pointer mt-8 shadow-md hover:shadow-lg">
          Get in
        </button>
      </form>
    </div>
